...
请定期使用页面内的“导出完整备份”保存数据；完整备份包含酒店、城市/目的地、铁路和航班/路线四类当前浏览器数据。部署前请确认要发布的数据已经脱敏。


## 首次使用

首次打开 `app.html` 时，页面会显示 public 模板提示，说明这是纯前端应用、数据默认保存在当前浏览器本地、不会上传用户数据到服务器，并提醒定期导出备份。

你可以使用“一键清空示例数据”把酒店、城市、铁路和航班示例清空。清空状态会写入当前浏览器本地，刷新后仍保持为空。需要重新查看模板效果时，可以使用“恢复示例数据”回到项目内置示例。
## 如何替换示例数据
