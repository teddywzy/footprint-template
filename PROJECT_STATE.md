...
模板数据全部为虚构示例，仅用于演示页面结构和功能。使用者应替换为自己的数据，并在公开部署前自行确认数据已脱敏。
...
3. 本项目是纯前端应用，默认不会上传用户数据到服务器；浏览器本地编辑数据默认保存在 localStorage，需定期导出备份。
4. 发布到 GitHub Pages 前，应确认数据文件只包含可公开展示的信息。
5. Leaflet 仍通过 CDN 引入；如需改成本地 vendor，应单独处理。
